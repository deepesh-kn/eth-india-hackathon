...
contract Executor {
    ...
    function executeSha() public returns (bytes32){
    ...
    function executeTx(
        ...
        bytes32 h = sha3(byte(0x19), byte(0), this, worker, "123", destination, data);
        bytes32 hashWithPrefix = keccak256("\x19Ethereum Signed Message:\n32",h);
        emit HashWithPrefix(hashWithPrefix);
    ...
    function getByte1() public pure returns(byte){
        return(byte(0x19));
    }



    function getByte2() public pure returns(byte){
        return(byte(0));
    }
